...
const convertApiItemToFrontend = (apiItem: WishlistItemAPI): WishlistItem => ({
  ...
  trend: 'flat', // Will be calculated based on price history
...
export const useWishlist = () => {
  ...
  const addItem = async (newItem: WishlistItem, collectionId?: number): Promise<boolean> => {
    try {
      ...
      const createdItem = await apiService.createWishlistItem(apiRequest);
      const frontendItem = convertApiItemToFrontend(createdItem);
      
      setItems(prevItems => [frontendItem, ...prevItems]);
